...
void PairSPHTaitwater::compute(int eflag, int vflag) {
  ...
  double rsq, tmp, wfd, delVdotDelR, mu, deltaE;
    double T, realViscosity , Ap, Bp, Cp, Dp;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (delVdotDelR < 0.) {
	     ...
	     Ap=0.00183;
	     Bp=1879.9;
             T=(e[i]-E0[itype])/(imass*4200)+T0[itype];
	       //if (T> 273.15){
	          //printf("i =%d, T=%f, itype=%d, E0[i]=%f, T0=%f, e[i]=%f,  imass=%f.\n", i, T, E0[itype], T0[itype], itype, e[i],  imass);
	         // }
	     realViscosity =Ap*pow(0.1,6)*exp(Bp/T);
	        // realViscosity =Ap*exp(Bp/T);
	    //realViscosity =realViscosity /rho[i];
	     realViscosity =realViscosity *8/(h*soundspeed[itype]);
	    	
         //********realv
	  //realViscosity = 1.856e-11*exp (4209/T + 0.04527*T-3.376e-5 *T *T);
	  //realViscosity = pow(100, 2.5-log10(Ti))-0.7;
	  //realViscosity =Ap*pow(0.1,6)*exp(Bp/T);
          //*******add
          fvisc = -realViscosity   * (soundspeed[itype] + soundspeed[jtype]) * mu / (rho[i] + rho[j]);
...
void PairSPHTaitwater::allocate() {
  ...
  memory->create(B, n+1 , "pair:B");
   memory->create(T0, n+1, "pair:T0");
  memory->create(E0, n+1, "pair: E0");
...
//add*******************
  double T_one = force->numeric(FLERR,arg[6]);
    double  E_one = force->numeric(FLERR,arg[7]);
/*
    int i, ii, inum;
    int *ilist;
   inum = list->inum;
